...
Vor der Merkmalsberechnung prüft das System die Überdeckung gleichzeitig erkannter Personen. Überschreitet die Schnittfläche relativ zur kleineren Bounding Box den konfigurierten Grenzwert, werden für beide Spuren weder Initialkandidaten noch Profilupdates erzeugt. Anschlie{\ss}end bleibt die Merkmalsberechnung für eine kurze Abkühlphase gesperrt. Bei einer noch unbekannten Spur wird zusätzlich der bisherige Initialpuffer verworfen, damit Ausschnitte vor und nach einer Kreuzung nicht gemeinsam ein Profil bilden. Die genaue Berechnung steht in Anhang~\ref{app:profile-details}.
...
Ein Initialkandidat muss zunächst die Gesamtqualitätsschwelle $Q\geq q_{\mathrm{init}}$ erfüllen. Zusätzlich gelten eigene Mindestgrenzen, die unabhängig vom Gesamtwert $Q$ geprüft werden: Die Schärfe muss mindestens $s_{\mathrm{init}}$ erreichen, die Bewertung des Seitenverhältnisses mindestens $a_{\mathrm{init}}$. Berührt die ursprüngliche Bounding Box einen Bildrand, gilt au{\ss}erdem die strengere Schärfegrenze $s_{\mathrm{Rand}}$. Gleichzeitig nimmt der Randteilwert $R$ in der Berechnung von $Q$ den Wert $\rho_{\mathrm{Rand}}$ an. Die harte Randschärfegrenze übernimmt dabei den wesentlichen Ausschluss unscharfer Randansichten; der verringerte Randteilwert senkt $Q$ ergänzend. Eine gute Grö{\ss}e oder Helligkeit kann einen offensichtlich unscharfen oder untypisch breiten Ausschnitt dadurch nicht allein über $Q$ ausgleichen. Die konkreten Parameterwerte wurden in den Pilotläufen festgelegt und stehen in Tabelle~\ref{tab:pilot-grid}. Die Berechnung beschreibt Anhang~\ref{app:profile-details}.

Erst wenn die für die jeweilige Konfiguration festgelegten Grenzen erfüllt sind, berechnet OSNet-x1.0 ein 512-dimensionales Embedding~\cite{zhou2019osnet}, dessen Gewichte aus dem Training auf MSMT17 stammen~\cite{torchreid-modelzoo}. Die Torchreid-Vorverarbeitung skaliert den RGB-Ausschnitt auf $256\times128$ Pixel, wandelt ihn in einen Tensor um und normalisiert die Kanäle mit den ImageNet-Mittelwerten $(0{,}485,0{,}456,0{,}406)$ und Standardabweichungen $(0{,}229,0{,}224,0{,}225)$. Eigenes Training oder Fine-Tuning findet nicht statt.

Vor der ersten Zuordnung sammelt das System fünf akzeptierte Ausschnitte derselben Spur. Die Frameindizes zweier aufeinanderfolgend angenommener Ausschnitte müssen sich um mindestens drei unterscheiden. Bei 30 FPS entspricht dies 0,10 Sekunden; fünf frühestmöglich angenommene Ausschnitte umfassen damit 0,40 Sekunden. Der Abstand sorgt nur für eine geringe zeitliche Streuung und garantiert keine unabhängigen Ansichten. Abgelehnte Ausschnitte verändern den Frameindex des zuletzt angenommenen Kandidaten nicht. Eine Höchstspanne für den Puffer gibt es nicht. Erreicht eine Spur vor einem Überlappungsreset oder dem Videoende keine fünf Kandidaten, bleibt sie ohne Personenkennung.
...
\label{eq:fusion}
\end{equation}
Das Mindestgewicht $0{,}05$ ist eine defensive Untergrenze für ungültig niedrige externe Gewichte. Bei den hier geometrisch gültigen und detektierten Ausschnitten liegt $Q$ bereits darüber, sodass die Untergrenze in den berichteten Läufen keine praktische Wirkung hat. Die Division durch die Gewichtssumme ändert wegen der anschlie{\ss}enden Normalisierung nicht die Richtung von $\bar z$, beschreibt aber den verwendeten gewichteten Mittelwert.
...
\label{eq:cosine}
\end{equation}
Bereits im aktuellen Frame sichtbare Personenkennungen werden dabei von der Auswahl ausgeschlossen, um Doppelzuweisungen zu vermeiden.
Erreicht das ähnlichste Profil den Matching-Schwellwert $\tau_{\mathrm{match}}$, wird seine Personenkennung der Spur zugeordnet. Andernfalls wird ein neues Profil mit einer neuen Personenkennung angelegt und der Spur zugeordnet. Ein zusätzlicher Mindestabstand zum zweitbesten Profil wird nicht geprüft. Neben dem Ergebnis werden auch das ähnlichste geprüfte Profil, sein Score und der Ablehnungsgrund protokolliert. Dadurch bleibt ein knapp verfehlter Profilabgleich nachträglich nachvollziehbar.

Im ersten Fall wird das zusammengeführte Embedding zusätzlich gegen die Update-Ähnlichkeitsschwelle $\tau_{\mathrm{upd}}$ aus Abschnitt~\ref{sec:profile-update} geprüft, bevor die fünf gewichteten Einzelbeiträge in das bestehende Profil einflie{\ss}en. In B0 sind $\tau_{\mathrm{match}}$ und $\tau_{\mathrm{upd}}$ identisch. Die zweite Prüfung kann nach einem erfolgreichen Match daher nicht scheitern und bietet dort keinen zusätzlichen Schutz. Allgemein bleiben beide Parameter getrennt konfigurierbar. Die Ähnlichkeit eines aus fünf Ausschnitten fusionierten Initialvektors und die Ähnlichkeit eines einzelnen Updateausschnitts folgen zudem nicht zwingend derselben Verteilung; gleiche Zahlenwerte sind deshalb nicht automatisch gleich streng.
...
Wird ein Update angenommen, speichert das System die Beobachtungen als kumulative gewichtete Summe. Im Profil liegen die gewichtete Embeddingsumme $E$ und die Gewichtssumme $W$. Für ein neues Embedding $z$ mit Gewicht $w=\max(Q,0{,}05)$ gilt
\begin{equation}
...
\end{equation}
Beim Anlegen eines Profils werden $E$ und $W$ mit Summe und Gewichtssumme der fünf Initialbeobachtungen initialisiert. Bei einem Match wird derselbe Fünferblock als Ganzes zum bestehenden Profil addiert, sofern $\tau_{\mathrm{upd}}$ erfüllt ist. Da $\operatorname{norm}(E/W)=\operatorname{norm}(E)$ gilt, wird $W$ für die Richtungsberechnung nicht benötigt; es hält jedoch die kumulierte Gewichtung für spätere Ergänzungen nachvollziehbar. Die fünf Initialbeobachtungen bleiben rechnerisch enthalten, ihr relativer Einfluss nimmt mit jedem weiteren Update ab. Ohne Begrenzung oder Vergessensfaktor kann das Profil nach vielen Updates daher von späteren Beobachtungen dominiert werden.
...
Initialbeobachtungen / Abstand & 5 / 3 Frames \\
